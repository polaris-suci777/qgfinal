...
public class QuestionDao {
    public Question createQuestion(Question question) throws Exception{
        ...
        try {
            ...
            String sql = "INSERT INTO question (id, course_name, type,content,choiceA,choiceB,choiceC,choiceD,answer) VALUES (null,?,?,?,?,?,?,?,?)";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, question.getCourse_name());
            stmt.setString(2, question.getType());
            stmt.setString(3, question.getContent());
            stmt.setString(4, question.getChoiceA());
    ...
    public List<Question> SelectAllByCourse_name(String course_name) throws SQLException {
        Connection conn;
        conn = DruidUtils.getConnection();
        String sql = "select * from question WHERE course_name = ?;";
        //3. 获取stmt对象
        PreparedStatement stmt = conn.prepareStatement(sql);
        //4. 设置参数
...
//            String course_name= rs.getString("course_name");
    ...
    public void update(Question question){
        ...
        try {
            conn = DruidUtils.getConnection();
            //2. 定义SQL
            String sql ="update question set course_name = ?,type=?,content = ?,choiceA = ?,choiceB = ?,choiceC = ?,choiceD = ?,answer=? where id = ?";
            //3. 获取stmt对象
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, question.getCourse_name());
            stmt.setString(2, question.getType());
