...
public class Student_answerDao {
    public Student_answer createStudent_answer(Student_answer student_answer) throws Exception {
        ...
        try {
            ...
            String sql = "INSERT INTO student_answer (id, course_name,content ,student_name,answer,is_correct,grade) VALUES (null,?,?,?,?,?,?)";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, student_answer.getCourse_name());
            stmt.setString(2, student_answer.getContent());
            stmt.setString(3, student_answer.getStudent_name());
            stmt.setString(4, student_answer.getAnswer());
            stmt.setString(5, student_answer.getIs_correct());
            stmt.setString(6, student_answer.getGrade());
            stmt.executeUpdate();
    ...
    public List<Student_answer> SelectAllByCourse_name(String course_name) throws SQLException {
        Connection conn;
        conn = DruidUtils.getConnection();
        String sql = "select * from student_answer WHERE course_name = ?;";
        //3. 获取stmt对象
        PreparedStatement stmt = conn.prepareStatement(sql);
        //4. 设置参数
    ...
    public Student_answer selectById(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                student_answer.setContent(rs.getString("content"));
    ...
    public void update(Student_answer student_answer) {
        ...
        try {
            conn = DruidUtils.getConnection();
            //2. 定义SQL
            String sql = "update student_answer set course_name = ?,content=?,student_name = ?,answer = ?,is_correct = ?,grade=? where id = ?";
            //3. 获取stmt对象
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, student_answer.getCourse_name());
            stmt.setString(2, student_answer.getContent());
            ...
            stmt.setString(5, student_answer.getIs_correct());
            stmt.setString(6, student_answer.getGrade());
            stmt.setInt(7, student_answer.getId());
            stmt.executeUpdate();
            if (stmt != null) stmt.close();
            if (conn != null) conn.close();;;
        } catch (SQLException e) {
